...
# Pose 用の検出・描画関数
def process_frame_pose(frame, pose):
    """
    入力画像（NumPy配列）に対して MediaPipe Pose で検出し、
    骨格を描画した画像と検出結果を返す。
    """
...
# Hands 用の検出・描画関数
def process_frame_hands(frame, hands):
    """
    入力画像（NumPy配列）に対して MediaPipe Hands で検出し、
    手のランドマークを描画した画像と検出結果を返す。
    """
...
if mode == "画像アップロード":
    ...
    if uploaded_file is not None:
        ...
        if detection_type == "骨格検出":
            ...
            # ランドマーク情報は results.pose_landmarks.landmark
            landmark_info = []
            x_coords = []
            y_coords = []
            text_coords = []
            ...
            # ランドマーク情報は results.multi_hand_landmarks
            landmark_info = []
            x_coords = []
            y_coords = []
            text_coords = []
        ...
        # 左右カラムに分割して表示：左にアップロード画像（オリジナルサイズ）、右に散布図
        ...
        st.write("モード：画像アップロード を選択してください。")
